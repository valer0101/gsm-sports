...
async function fetchInitial(sport: string): Promise<PaginatedResponse<Tournament> | undefined> {
  try {
    ...
    const res = await fetch(`${apiUrl}/tournaments?limit=12&sport=${sport}`, { next: { revalidate: 60 } });
...
export default async function SportTournamentsPage({ params }: { params: Promise<{ sport: string }> }) {
