...
export function PlaceGroup({
  ...
  const ordinalLabel = (n: number): string => {
    if (n === 1) return t('ordinal_1st');
    if (n === 2) return t('ordinal_2nd');
    if (n === 3) return t('ordinal_3rd');
    return t('ordinal_nth', { n });
  };

  const rewardCountLabel = (count: number) =>
    t(count === 1 ? 'prize_n_rewards_one' : 'prize_n_rewards_other', { count });
  ...
  const ordLabel = ordinalLabel(place);
      ...
      <div className="pl-5 pr-4 py-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-bold">{ordLabel}</div>
              <div className="text-[11px] text-[var(--color-text-muted)]">
                {rewardCountLabel(rewards.length)}
              </div>
            </div>
          </div>
          <button
            ...
            aria-label={t('prize_remove_place', { place: ordLabel })}
            title={t('prize_remove_place', { place: ordLabel })}
          >
            {Icon.trash()}
          </button>
        </div>
        ...
        >
          ...
          {t('prize_add_reward_to_place', { place: ordLabel })}
        </button>
      </div>
    </div>
  );
